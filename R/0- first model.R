...
time <- 1000
ensemble <- 1
# etha felan baraye hame hamsaye ha yeksan dar nazar gerefte shode, vali baadan mishe ba tavajo be lag fargh bokone
# ounam be do soorat ke bara har miangin hamsaye ha bar asase lag ye noise beBne, ya kollan nahayatan ye noise e 
# nahayi bara kolle porose beBne. too in model e aval farz mikonim ke yedoone akhar beBne (intrinsic o extrinsic)

ethas <- seq(0.05, 1, by = 0.05)
means <- c(0, 5, 10, 20, 50, 100, 250, 500, 1000)
sds <- c(1, 5, 10, 20, 50, 100, 250, 500, 1000)
...
plot_name <- vector()
k <- 1378
for (etha in c(0.9, 0.95, 1)) {
  for (mean_gaussian in means) {
    for (sd_gaussian in sds) {
      ensembles <- list()
      ...
      plot_name[k] <- paste0("data/plots/", etha,"-", mean_gaussian, "-", sd_gaussian, ".jpg")
      plot_list[[k]] = ggplot(order_overall, aes(1:1000, overall)) +
...
save(plot_list, plot_name, file = "data/plots.rda")

k <- k - 1
for (i in 1:k) {
  dir <- plot_name[i]
  jpeg(dir, width = 1000, height = 1000)
  print(plot_list[[i]])
  dev.off()
}
